...
func findNodesThatFit(
	...
	extenders []algorithm.SchedulerExtender,
	metaProducer algorithm.MetadataProducer,
) ([]*api.Node, FailedPredicateMap, error) {

	pFactory := algorithm.EmptyMetadataProducer
	if metaProducer == nil {
		glog.Errorf("Nil meta producer: This means at least some predicates will not function properly.")
	}
	...
	if len(predicateFuncs) == 0 {
		...
		metaDataProducer := pFactory(pod, nodeNameToInfo)
		checkNode := func(i int) {
			nodeName := nodes[i].Name
			// pFactory takes a pod, node info, and creates a black-box type which can be used by the predicate.
			fits, failedPredicates, err := podFitsOnNode(pod, metaDataProducer, nodeNameToInfo[nodeName], predicateFuncs)
			if err != nil {
