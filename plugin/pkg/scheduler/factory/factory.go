...
// ConfigFactory is the default implementation of the SchedulerConfiguratinUtility interface.
...
// NewConfigFactory initializes the default implementation of a SchedulerConfigurationUtility.  To encourage eventual privatization of the struct type, we only
// return the interface.
func NewConfigFactory(client clientset.Interface, schedulerName string, hardPodAffinitySymmetricWeight int, failureDomainsCString string) scheduler.SchedulerConfigurationUtility {
	...
	c := &ConfigFactory{
		...
		failureDomains:                 strings.Split(failureDomainsCString, ","),
	}
...
func (f *ConfigFactory) CreateFromKeys(predicateKeys, priorityKeys sets.String, extenders []algorithm.SchedulerExtender) (*scheduler.Config, error) {
	...
	podBackoff := scheduler.CreateDefaultPodBackoff()
...
func (factory *ConfigFactory) MakeDefaultErrorFunc(backoff *scheduler.PodBackoff, podQueue *cache.FIFO) func(pod *v1.Pod, err error) {
