...
// parse is just a util for test readability.
...
func TestURLBackoffFunctionality(t *testing.T) {
	...
	seconds := []int{0,
		...
		1, 2}
	returnCodes := []int{
		429, 500, 501, 502, 300 /*no backoff for 200,300 and many other resp codes*/,
		500, 501, 502,
		}
	...
	/**
	 * The final request is a 200, so, the last backoff should be 0.
	 */

	//Now confirm that after all those queries, clearing works.
	myBackoff.Reset()
	if myBackoff.CalculateBackoff(parse("http://1.2.3.4:100")) > 0 {
		t.Errorf("Backoff was not cleared properly")
	}

	//Add entries again.
	myBackoff.UpdateBackoff(parse("http://1.2.3.4:100"), nil, 500)

	if myBackoff.CalculateBackoff(parse("http://1.2.3.4:100")) == 0 {
		t.Errorf("Backoff not working after clearing")
	}

	myBackoff.Disable()
	if myBackoff.CalculateBackoff(parse("http://1.2.3.4:100")) != 0 {
		t.Errorf("Backoff was not properly disabled.")
	}
