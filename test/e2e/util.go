...
type validatorFn func(c *client.Client, podId string) (interface{}, error)

/**
* Abstracted version of the original validateController function.
*
* Takes a "containername": this is grepped for.
* Takes a "testname":  which gets bubbled up to the logging/failure messages if errors happen.
* Takes a "validator" function: This function is given a podID and a client, and it can do some specific validations that way.
 */
func validateController(c *client.Client, image string, replicas int, containername string, testname string, validator validatorFn) {
	...
	//Reaching here means that one of more checks failed multiple times.  Assuming its not a race condition, something is broke.
