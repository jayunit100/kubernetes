...
var _ = Describe("Networking", func() {
	...
	BeforeEach(func() {
		...
		namespaceObj, err = createGinkgoNS("nettest", c)
		Expect(err).NotTo(HaveOccurred())

	})

	AfterEach(func() {
		By(fmt.Sprintf("Destroying namespace for this suite %v", namespaceObj.Name))
	...
	It("should function for intra-pod communication", func() {
		...
		By(fmt.Sprintf("Creating a service named [%s] in namespace %s", svcname, namespaceObj.Name))
		svc, err := c.Services(namespaceObj.Name).Create(&api.Service{
			ObjectMeta: api.ObjectMeta{
				Name: svcname,
				Labels: map[string]string{
					"name": svcname,
		...
		defer func() {
			...
			if err = c.Services(namespaceObj.Name).Delete(svc.Name); err != nil {
		...
		podNames := LaunchNetTestPodPerNode(nodes, svcname, c, namespaceObj.Name)
		...
		defer func() {
			...
			for _, podName := range podNames {
				if err = c.Pods(namespaceObj.Name).Delete(podName); err != nil {
		...
		for _, podName := range podNames {
			err = waitForPodRunningInNamespace(c, podName, namespaceObj.Name)
			Expect(err).NotTo(HaveOccurred())
		}
		...
		for i := 0; i < maxAttempts && !passed; i++ {
			...
			case "fail":
				if body, err = c.Get().
					Namespace(namespaceObj.Name).Prefix("proxy").
					Resource("services").
