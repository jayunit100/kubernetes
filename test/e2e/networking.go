...
var _ = Describe("Networking", func() {
	...
	for _, svcSoak := range serviceSoakTests {
		...
		It(fmt.Sprintf("should function for intrapod communication between all hosts in %v parallel services", serviceNum),
			func() {
...
func pollPeerStatus(serviceDoneChannel chan string, f *Framework, svc *api.Service, pollTimeoutSeconds time.Duration) {
	...
	expectNoError(wait.Poll(2*time.Second, pollTimeoutSeconds, func() (bool, error) {
		...
		//pass/fail the test based on if the container's response body, as to whether or not it was able to find peers.
		switch {
...
func runNettestSingle(nodes *api.NodeList, port int) {
	...
	for _, podName := range podNames {
		...
		pollPeerStatus(sem, f, svc, timeoutSeconds)
	}
...
func runNetTest(timeoutSeconds time.Duration, f *Framework, ports []int, nettestVersion string) {
	...
	sem := make(chan string, len(ports))
	for p := range ports {
		runNettestSingle(nodes, ports[p])
	}
	...
	for pReturned := range ports {
		...
		Logf("... Another service has successfully been discovered: %v ( %v ) ", pReturned, <-sem)
	}
