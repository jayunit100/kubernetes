...
/**
* TODO this should leverage test utils file, rather than using its own waitForGuestbookResp implementation.
 */
...
/**
*  Get data for the update demo
 */
func getUDData(c *client.Client, podID string) (interface{}, error) {
	body, err := c.Get().
		Prefix("proxy").
		Resource("pods").
		Name(podID).
	...
	if strings.Contains(data.image, "update-demo") {
		...
		return &data, errors.New("data served up in container is innaccurate")
	}
}
