...
func (k *Kubernetes) GetPodFromCache(ns string, name string) (*v1.Pod, error) {
...
func (k *Kubernetes) GetPodsFromCacheByKeyVal(ns string, key string, val string) ([]v1.Pod, error) {
	k.mutex.Lock()
...
func (k *Kubernetes) GetPod(ns string, name string) (*v1.Pod, error) {
	...
	return kubePod, errors.Wrapf(err, "unable to get pod %s/%s", ns, name)
}
...
func (k *Kubernetes) deleteNamespaces(namespaces []string) error {
	for _, ns := range namespaces {
